...
while getopts ':f:w:o:b:n:e:' opt ; do
    case "$opt" in
    f) FASTA="${OPTARG}" ;;
    w) WORKDIR="${OPTARG}" ;;	
    o) OUTDIR="${OPTARG}"
...
{   
    ...
    cmd="/home/theoh/git_master/read_simulator/venv/bin/python "
    cmd+="$script -f $RG_UPDATED_BAM -r $FASTA -o $TUMOR_BAM -v "
    cmd+="$BAMSUREON_BED --mindepth 15 -p $NUM_PROCESSES"
